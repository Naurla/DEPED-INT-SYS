...
<body class="bg-gray-100 flex h-screen overflow-hidden" 
    ...
    <aside class="bg-[#b91c1c] text-white transition-all duration-300 flex flex-col shadow-xl" :class="sidebarOpen ? 'w-64' : 'w-20'">
        <div class="p-6 border-b border-red-800 flex items-center justify-between">
            <h1 class="font-bold tracking-tighter overflow-hidden whitespace-nowrap" x-show="sidebarOpen">DEPED ADMIN</h1>
        ...
        <nav class="flex-grow p-4 space-y-2 text-sm overflow-y-auto">
            <a href="<?php echo e(route('admin.dashboard')); ?>" class="flex items-center space-x-3 px-4 py-3 hover:bg-red-700 rounded-lg transition-colors">
            ...
            <a href="<?php echo e(route('admin.advisory.index')); ?>" class="flex items-center space-x-3 px-4 py-3 bg-red-800 rounded-lg font-bold">
        ...
        <main class="flex-grow p-8 overflow-y-auto bg-gray-50">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div class="p-6 bg-gray-50 border-b border-gray-100">
                    <form action="<?php echo e(route('admin.advisory.index')); ?>" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <input type="text" name="search" placeholder="Search titles..." value="<?php echo e(request('search')); ?>" 
                               class="border border-gray-300 p-2 rounded-lg focus:ring-2 focus:ring-red-600 outline-none bg-white text-sm">
                        <input type="date" name="start_date" value="<?php echo e(request('start_date')); ?>" class="border border-gray-300 p-2 rounded-lg text-sm bg-white">
                        <input type="date" name="end_date" value="<?php echo e(request('end_date')); ?>" class="border border-gray-300 p-2 rounded-lg text-sm bg-white">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-6 py-4 font-semibold text-gray-700">
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <span><?php echo e($advisory->title); ?></span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-gray-500"><?php echo e($advisory->created_at->format('M d, Y')); ?></td>
                                <td class="px-6 py-4">
                                    <span class="px-2 py-1 rounded-full text-[10px] font-bold <?php echo e($advisory->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'); ?>">
                                        <?php echo e($advisory->is_active ? 'PUBLISHED' : 'ARCHIVED'); ?>

                                    </span>
                                </td>
                                <td class="px-6 py-4 text-center space-x-3">
                                    <button @click="openEdit(<?php echo e($advisory); ?>)" class="text-blue-600 hover:underline font-bold text-xs uppercase">Edit</button>
                                    <button @click="confirmDelete(<?php echo e($advisory->id); ?>)" class="text-red-600 hover:underline font-bold text-xs uppercase">Delete</button>
                                </td>
                            </tr>
    ...
    <div x-show="uploadModal" class="fixed inset-0 z-50 overflow-y-auto" x-cloak>
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="bg-white rounded-xl shadow-xl overflow-hidden z-50 w-full max-w-md transform transition-all">
                ...
                <form :action="editMode ? '/admin/advisories/' + advisoryId : '<?php echo e(route('advisories.store')); ?>'" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
                    <?php echo csrf_field(); ?>
...
</html><?php /**PATH C:\Users\Noltan\Desktop\DEPED INT SYS\example-app\resources\views/admin/advisories/index.blade.php ENDPATH**/ ?>