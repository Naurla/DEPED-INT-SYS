...
<div class="bg-gray-100 border-b border-gray-200 w-full overflow-hidden">
    <div class="container mx-auto px-4 md:px-20 max-w-10xl py-3 text-xs sm:text-sm text-gray-600 overflow-x-auto whitespace-nowrap hide-scroll">
        ...
        <a href="{{ route('enrollment-statistics.index') }}" class="hover:text-[#003366] transition">Enrollment Statistics</a>
        <span class="mx-2">></span>
...
<div class="container mx-auto px-4 md:px-20 max-w-10xl py-8 md:py-12 w-full overflow-hidden min-h-screen">
    ...
    <div class="mb-8 md:mb-10 text-left w-full break-words">
        ...
        <a href="{{ route('enrollment-statistics.index') }}" class="text-[#a52a2a] hover:text-red-800 font-bold text-sm inline-flex items-center mb-6 uppercase tracking-wider transition-colors">
    ...
    @if($item->image_path && $item->file_path)
        ...
        <div class="mb-8 w-full flex flex-col items-center bg-gray-50 rounded-lg p-4 border border-gray-200 shadow-inner">
            <a href="{{ asset('storage/' . $item->file_path) }}" target="_blank" class="block w-full text-center group cursor-pointer" title="Click to view full document">
                ...
                <img src="{{ route('serve.image', $item->image_path) }}" alt="{{ $item->title }}" class="max-w-full h-auto mx-auto rounded shadow-sm border border-gray-300 group-hover:opacity-90 group-hover:shadow-md transition-all duration-300">
