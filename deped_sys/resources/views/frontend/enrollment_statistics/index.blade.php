...
<div class="container mx-auto px-4 md:px-20 max-w-10xl py-12 w-full min-h-screen">
    ...
    {{-- w-full applied here so it consumes the whole page width --}}
    <div class="w-full space-y-12">
        @forelse($items as $item)
            <div class="border-b border-gray-100 pb-10 last:border-0 group">
                
                {{-- Date and Title Heading --}}
                <a href="{{ route('enrollment-statistics.show', ['id' => $item->id]) }}" class="block mb-4">
                    <h2 class="text-[1.3rem] font-bold text-gray-900 uppercase tracking-tight hover:text-gray-700 transition-colors leading-snug">
                        {{ strtoupper($item->created_at->format('F d, Y')) }} - {{ strtoupper($item->title) }} @if($item->school_year) - SY {{ strtoupper($item->school_year) }} @endif
                    </h2>
                </a>
                
                {{-- Description Preview --}}
                <div class="text-[15px] text-gray-600 mb-6 leading-relaxed w-full">
                    {{ $item->title }} @if($item->content) - {{ Str::limit(strip_tags($item->content), 250) }} @endif
                </div>

                {{-- Action Row --}}
                <div class="flex items-center gap-6">
                    {{-- Read More Button --}}
                    <a href="{{ route('enrollment-statistics.show', ['id' => $item->id]) }}" 
