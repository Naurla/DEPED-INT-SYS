...
<div class="bg-gray-100 border-b border-gray-200 w-full overflow-hidden">
    <div class="container mx-auto px-4 md:px-20 max-w-10xl py-3 text-xs sm:text-sm text-gray-600 overflow-x-auto whitespace-nowrap hide-scroll">
        ...
        <a href="{{ route('als-stories.index') }}" class="hover:text-[#003366] transition">ALS Stories</a>
        <span class="mx-2">></span>
...
<div class="container mx-auto px-4 md:px-20 max-w-10xl py-8 md:py-12 w-full overflow-hidden min-h-screen">
    ...
    <div class="mb-8 md:mb-10 text-left w-full break-words">
        ...
        <a href="{{ route('als-stories.index') }}" class="text-[#a52a2a] hover:text-red-800 font-bold text-sm inline-flex items-center mb-6 uppercase tracking-wider transition-colors">
        ...
        <div class="flex flex-wrap items-center text-gray-500 font-semibold gap-x-6 gap-y-3 mb-8">
            ...
            {{-- Secure Image Download Link (If you want users to download the cover image) --}}
            @if($item->image_path)
                <a href="{{ route('serve.image', $item->image_path) }}" target="_blank" class="text-blue-600 hover:text-blue-800 hover:underline flex items-center whitespace-nowrap text-[13px] uppercase tracking-widest transition-colors font-bold" download>
                    <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
                    Download Image
                </a>
            @endif
        ...
        {{-- TEXT CONTENT (Maintained normal weight and whitespace-pre-line for readability) --}}
        @if($item->content)
            <div class="text-[15px] text-gray-700 leading-relaxed mb-10 max-w-4xl whitespace-pre-line">
                {{ $item->content }}
            </div>
        @endif
    ...
    @if($item->image_path && $item->file_path)
        ...
        <div class="mb-8 w-full flex flex-col items-center">
            <a href="{{ asset('storage/' . $item->file_path) }}" target="_blank" class="block w-full text-center group cursor-pointer" title="Click to view full document">
                
                <img src="{{ route('serve.image', $item->image_path) }}" alt="{{ $item->title }}" class="max-w-full h-auto mx-auto rounded shadow-sm border border-gray-200 group-hover:opacity-90 group-hover:shadow-md transition-all duration-300">
    ...
    @elseif($item->image_path)
        ...
        <div class="mb-8 w-full flex justify-center">
            <img src="{{ route('serve.image', $item->image_path) }}" alt="{{ $item->title }}" class="max-w-full h-auto rounded shadow-sm border border-gray-200">
        </div>
    ...
    <p class="text-gray-400 text-[11px] text-center mb-8 mt-12 font-bold uppercase tracking-widest">
        This story is published by DepEd Zamboanga City.
    </p>
