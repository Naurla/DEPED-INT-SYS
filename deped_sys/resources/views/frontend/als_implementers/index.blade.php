...
<div class="bg-gray-100 border-b border-gray-200 w-full overflow-hidden">
    <div class="container mx-auto px-4 md:px-20 max-w-10xl py-3 text-xs sm:text-sm text-gray-600 overflow-x-auto whitespace-nowrap hide-scroll">
        ...
        <span class="text-gray-900 font-bold">{{ $type_name ?? 'Featured ALS Implementers' }}</span>
    </div>
</div>
        ...
        <div class="space-y-16">
            @forelse($items as $item)
                <div class="group transition-all duration-300">
                    ...
                    <a href="{{ route('als-implementers.show', ['id' => $item->id]) }}" class="block">
                    ...
                    <div class="flex items-center gap-6">
                        <a href="{{ route('als-implementers.show', ['id' => $item->id]) }}" class="inline-block border border-gray-300 text-gray-600 px-6 py-2 text-xs font-bold uppercase tracking-widest hover:bg-gray-50 hover:text-gray-900 transition-colors rounded-sm shadow-sm">
                            Read More
                        </a>
                        {{-- Added the Posted Date to match the standardized listing layout --}}
