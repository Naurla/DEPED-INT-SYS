...
<div x-data="{ 
    ...
    editMaterial: {{ (old('form_type') === 'edit' && $errors->any()) ? Js::from([
        'id' => old('id'),
        'title' => old('title'),
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        <button @click="addModal = true" class="bg-red-700 hover:bg-red-800 text-white font-bold py-2.5 px-4 rounded-lg shadow transition-colors text-sm flex items-center shrink-0 uppercase tracking-wider">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 align-middle">
                                <div class="flex justify-end gap-3 items-center">
                                    <button @click="openEdit({{ collect($material)->toJson() }})" class="text-blue-600 hover:text-blue-800 font-bold text-xs uppercase hover:underline">Edit</button>
                                    <button @click="confirmDelete({{ $material->id }}, '{{ addslashes($material->title) }}')" class="text-red-600 hover:text-red-800 font-bold text-xs uppercase hover:underline">Delete</button>
                                </div>
                            </td>
                        </tr>
                    @empty
    ...
    <div x-show="addModal" x-cloak class="fixed inset-0 z-[90] flex items-center justify-center bg-black bg-opacity-60 px-4 backdrop-blur-sm transition-opacity">
        <div class="bg-white rounded-xl w-full max-w-5xl shadow-2xl overflow-hidden flex flex-col max-h-[95vh]" @click.away="addModal = false">
            ...
            <div class="bg-red-700 px-8 py-5 flex justify-between items-center text-white flex-shrink-0">
                <h3 class="font-bold text-2xl">Upload New Learning Material</h3>
                <button type="button" @click="addModal = false" class="hover:text-gray-200 text-4xl font-bold">&times;</button>
            </div>

            <form action="{{ route('admin.learning-materials.store') }}" method="POST" enctype="multipart/form-data" class="flex flex-col overflow-hidden min-h-0">
                @csrf
                ...
                <div class="p-8 space-y-6 overflow-y-auto custom-scrollbar flex-1">
                    <div>
                        ...
                        <input type="text" name="title" value="{{ old('form_type') === 'add' ? old('title') : '' }}" required placeholder="Enter material title..." class="w-full border border-gray-300 p-4 text-lg rounded-lg focus:ring-2 focus:ring-red-500 outline-none">
                        @if(old('form_type') === 'add') @error('title') <p class="text-red-500 text-base mt-1.5 font-medium">{{ $message }}</p> @enderror @endif
                    </div>

                    <div>
                        <label class="block text-gray-800 text-lg font-bold mb-2">Description <span class="text-red-500">*</span></label>
                        <textarea name="description" rows="6" required placeholder="Enter detailed description..." class="w-full border border-gray-300 p-4 text-lg rounded-lg focus:ring-2 focus:ring-red-500 outline-none resize-none">{{ old('form_type') === 'add' ? old('description') : '' }}</textarea>
                    ...
                    <div class="bg-gray-50 p-6 rounded-lg border border-gray-200">
                        ...
                        <p class="text-sm text-gray-500 mt-2 italic">Supported formats: PDF, Word, Excel, PowerPoint, CSV (Max: 20MB)</p>
                ...
                <div class="bg-gray-50 px-8 py-5 flex flex-row-reverse gap-4 items-center border-t border-gray-200 flex-shrink-0">
                    <button type="submit" class="bg-red-700 hover:bg-red-800 text-white font-bold py-3.5 px-10 rounded-lg shadow-md transition-colors text-lg">Upload Material</button>
    ...
    <div x-show="editModal" x-cloak class="fixed inset-0 z-[90] flex items-center justify-center bg-black bg-opacity-60 px-4 backdrop-blur-sm transition-opacity">
        <div class="bg-white rounded-xl w-full max-w-5xl shadow-2xl overflow-hidden flex flex-col max-h-[95vh]" @click.away="editModal = false">
            ...
            <div class="bg-red-700 px-8 py-5 flex justify-between items-center text-white flex-shrink-0">
                <h3 class="font-bold text-2xl">Edit Learning Material</h3>
                <button type="button" @click="editModal = false" class="hover:text-gray-200 text-4xl font-bold">&times;</button>
            </div>

            <form :action="`/admin/learning-materials/${editMaterial?.id}`" method="POST" enctype="multipart/form-data" class="flex flex-col overflow-hidden min-h-0">
                @csrf @method('PUT')
                <input type="hidden" name="form_type" value="edit">
                <input type="hidden" name="id" :value="editMaterial?.id">
                <input type="hidden" name="existing_file" :value="editMaterial?.file_path">
                <input type="hidden" name="remove_file" :value="removeFile ? '1' : '0'">

                <div class="p-8 space-y-6 overflow-y-auto custom-scrollbar flex-1">
                    <div>
                        <label class="block text-gray-800 text-lg font-bold mb-2">Title <span class="text-red-500">*</span></label>
                        <input type="text" name="title" x-model="editMaterial.title" required class="w-full border border-gray-300 p-4 text-lg rounded-lg focus:ring-2 focus:ring-red-500 outline-none">
                        @if(old('form_type') === 'edit') @error('title') <p class="text-red-500 text-base mt-1.5 font-medium">{{ $message }}</p> @enderror @endif
                    </div>

                    <div>
                        <label class="block text-gray-800 text-lg font-bold mb-2">Description <span class="text-red-500">*</span></label>
                        <textarea name="description" x-model="editMaterial.description" required rows="6" class="w-full border border-gray-300 p-4 text-lg rounded-lg focus:ring-2 focus:ring-red-500 outline-none resize-none"></textarea>
                    ...
                    <div class="bg-gray-50 p-6 rounded-lg border border-gray-200">
                        ...
                        <template x-if="editMaterial && editMaterial.file_path && !removeFile">
                            <div class="mt-3 flex items-center justify-between p-3 bg-red-50 border border-red-200 rounded-lg">
                                <span class="text-base text-red-800 font-bold truncate max-w-[400px]" x-text="'Current File: ' + editMaterial.file_path.split('/').pop()"></span>
    ...
    <div x-show="deleteModal" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center bg-black bg-opacity-60 px-4 backdrop-blur-sm transition-opacity">
        <div class="bg-white rounded-3xl shadow-2xl z-50 w-full max-w-md transform transition-all relative overflow-hidden p-8" @click.away="deleteModal = false">
            ...
            <div class="text-center">
                <h3 class="text-2xl font-bold text-gray-900 mb-2">Delete Material?</h3>
                <p class="text-gray-500 text-sm mb-5">
                    You are about to permanently delete this learning material:
                </p>
            ...
            <div class="flex gap-3">
                ...
                <form :action="`/admin/learning-materials/${deleteId}`" method="POST" class="flex-1 m-0 p-0">
                    @csrf 
                    @method('DELETE')
