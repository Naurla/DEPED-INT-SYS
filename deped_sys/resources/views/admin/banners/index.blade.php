...
<div x-data="{ 
    ...
    currentImagePath: '',
    deleteTitle: '',
    occupiedOrders: @json($occupiedOrders),
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        <button @click="bannerModal = true" class="bg-red-700 hover:bg-red-800 text-white font-bold py-2.5 px-4 rounded-lg shadow transition-colors text-sm uppercase tracking-wider flex items-center shrink-0">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-6">
        <form method="GET" action="{{ url()->current() }}" class="flex flex-col xl:flex-row gap-4 items-center justify-between">
            ...
            <div class="w-full xl:w-1/3 relative">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search image name..." class="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none text-sm transition-colors">
            </div>
            ...
            <div class="w-full xl:w-auto flex flex-col md:flex-row gap-3 items-center">
                ...
                <select name="sort" class="w-full md:w-40 py-2.5 px-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 outline-none text-sm bg-white text-gray-700 cursor-pointer" onchange="this.form.submit()">
                    ...
                    <option value="active" {{ request('sort') == 'active' ? 'selected' : '' }}>Active First</option>
                    <option value="inactive" {{ request('sort') == 'inactive' ? 'selected' : '' }}>Inactive First</option>
                </select>

                {{-- Clear Filters --}}
                @if(request('search') || request('month') || request('year') || (request('sort') && request('sort') !== 'order'))
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-100 text-gray-700">
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="px-6 py-10 text-center text-gray-500 italic">No banners found matching your criteria.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
