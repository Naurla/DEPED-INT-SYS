...
<div x-data="{ showDeleteModal: false, deleteAction: '', deleteTitle: '' }">
        ...
        <div class="xl:col-span-5 bg-white p-6 rounded-lg shadow-sm border border-gray-200 h-fit">
            ...
            <form action="{{ route('admin.division_structures.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="name" required placeholder="e.g., Office of the Schools Division Superintendent" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 text-sm">
                </div>
                ...
                <div class="border-t border-gray-100 pt-4 space-y-4">
                    ...
                    <div>
                        ...
                        <p class="text-[11px] text-gray-500 mb-2 leading-tight">Upload PDFs here. Once saved, copy the link from the list on the right, and paste it into the Content Text above as a link.</p>
                ...
                <button type="submit" class="w-full mt-3 bg-red-700 text-white py-2.5 px-4 rounded-lg hover:bg-red-800 transition-colors text-sm font-bold shadow-sm tracking-wide">
                    Save Entry
                </button>
            </form>
        </div>
                                ...
                                <ul class="list-none space-y-2 mt-2 text-sm text-gray-700">
                                    @foreach($struct->pdf_documents as $index => $pdf)
                                        <li class="flex items-center justify-between bg-white px-3 py-2 rounded-lg border border-gray-200 shadow-sm">
                                            ...
                                            {{-- PDF specific delete NOW HOOKED INTO THE MODAL! --}}
    ...
    <div x-show="showDeleteModal" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center bg-black bg-opacity-60 px-4 backdrop-blur-sm transition-opacity" style="display: none;">
        <div class="bg-white rounded-2xl p-8 shadow-2xl z-50 w-full max-w-sm transform transition-all relative" @click.away="showDeleteModal = false">
            ...
            <p class="text-gray-500 text-sm mb-6 text-center">Are you sure you want to delete <br><strong class="text-gray-800 break-words" x-text="deleteTitle"></strong>? <br>This action cannot be undone.</p>
            ...
            <div class="flex space-x-3 border-t border-gray-100 pt-4">
                ...
                <form :action="deleteAction" method="POST" class="flex-1 m-0 p-0">
                    @csrf 
                    @method('DELETE')
                    <button type="submit" class="w-full px-4 py-2.5 bg-red-600 text-white rounded-xl font-bold text-sm hover:bg-red-700 shadow-lg shadow-red-200 transition-colors">
                        Delete
                    </button>
                </form>
            </div>
