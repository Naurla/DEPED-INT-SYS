            ...
            <table class="w-full text-left text-sm border-collapse">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 border-b transition-colors">
                        <td class="p-4 font-semibold text-gray-800">{{ $implementer->title }}</td>
                        
                        <td class="p-4 text-gray-500">
                            @if($implementer->image_path)
                                <a href="{{ asset('storage/' . $implementer->image_path) }}" target="_blank" title="{{ basename($implementer->image_path) }}" class="text-blue-600 font-bold hover:text-blue-800 hover:underline flex items-center text-xs whitespace-nowrap">
                                    <svg class="w-4 h-4 mr-1 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                                    <span class="max-w-[150px] truncate">{{ basename($implementer->image_path) }}</span>
                                </a>
                            @else
                        ...
                        <td class="p-4 text-gray-500">
                            @if($implementer->file_path)
                                <a href="{{ asset('storage/' . $implementer->file_path) }}" target="_blank" title="{{ basename($implementer->file_path) }}" class="text-red-600 font-bold hover:text-red-800 hover:underline flex items-center text-xs whitespace-nowrap">
                                    ...
                                    <span class="max-w-[150px] truncate">{{ basename($implementer->file_path) }}</span>
                                </a>
                            @else
                                <span class="text-gray-400 italic text-xs">N/A</span>
                            @endif
                    ...
                    @empty
                    <tr>
                        <td colspan="4" class="p-6 text-center text-gray-500">No records found.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div x-show="uploadModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 px-4 backdrop-blur-sm transition-opacity">
        <div class="bg-white rounded-xl w-full max-w-2xl shadow-2xl overflow-hidden" @click.away="uploadModal = false">
            <div class="bg-red-700 px-6 py-4 flex justify-between items-center text-white">
                <h3 class="font-bold text-lg" x-text="editMode ? 'Edit Entry' : 'Upload New Entry'"></h3>
            ...
            <form :action="editMode ? '/admin/als-implementers/' + implementerId : '{{ route('admin.als-implementers.store') }}'" 
                ...
                <div>
                    ...
                    <input type="text" name="title" x-model="formData.title" placeholder="e.g., Juan Dela Cruz - January 2024" required class="w-full border border-gray-300 p-2.5 text-sm rounded-lg focus:ring-2 focus:ring-red-500 outline-none">
                </div>
                ...
                <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-100">
                    ...
                    <button type="submit" class="px-5 py-2 text-sm bg-red-700 hover:bg-red-800 text-white font-bold rounded-lg shadow-sm transition-colors" x-text="editMode ? 'Save Changes' : 'Save Entry'"></button>
                </div>
            </form>
        </div>
    </div>
