...
<div x-data="{ 
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="p-6 border-b border-gray-100 flex justify-between items-center">
            <h3 class="font-bold text-gray-800 text-xl">School Governance and Operations Division</h3>
            ...
            <table class="w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-6 py-4">
                            <img src="{{ route('serve.image', $sgod->image_path) }}" class="h-12 w-auto object-cover rounded border border-gray-200">
                        </td>
                        <td class="px-6 py-4 font-semibold text-gray-700">{{ $sgod->title }}</td>
                        <td class="px-6 py-4 text-center space-x-3">
                            <button @click="openEdit({{ collect($sgod)->toJson() }})" class="text-blue-600 font-bold uppercase text-xs hover:underline">Edit</button>
                            <button @click="confirmDelete({{ $sgod->id }})" class="text-red-600 font-bold uppercase text-xs hover:underline">Delete</button>
                        </td>
                    </tr>
                    @empty
                    <tr>
            ...
            <div class="bg-white rounded-xl shadow-xl overflow-hidden z-50 w-full max-w-lg">
                ...
                <form :action="editMode ? '/admin/sgod/' + itemId : '{{ route('admin.sgod.store') }}'" 
                    ...
                    <div>
                        ...
                        <input type="text" name="title" x-model="formData.title" placeholder="e.g., SGOD Organizational Chart 2024" required class="w-full border rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-red-500">
                    </div>
        ...
        <div class="flex items-center justify-center min-h-screen px-4 text-center">
            ...
            <div class="bg-white rounded-2xl p-8 shadow-2xl z-[70] w-full max-w-sm">
                ...
                <div class="flex space-x-3">
                    ...
                    <form :action="'/admin/sgod/' + itemId" method="POST" class="flex-1">
                        @csrf @method('DELETE')
