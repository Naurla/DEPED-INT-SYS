        ...
        <div class="bg-white rounded-xl w-full max-w-2xl shadow-2xl overflow-hidden" @click.away="uploadModal = false">
            ...
            <form :action="editMode ? '/admin/cid/' + itemId : '{{ route('admin.cid.store') }}'" 
                ...
                <div>
                    ...
                    <input type="text" name="title" x-model="formData.title" placeholder="e.g., CID Organizational Chart 2024" required class="w-full border border-gray-300 p-2.5 text-sm rounded-lg focus:ring-2 focus:ring-red-500 outline-none">
                </div>
                ...
                <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-100">
                    <button type="button" @click="uploadModal = false" class="px-5 py-2.5 text-sm font-bold text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors">Cancel</button>
                    <button type="submit" class="px-5 py-2.5 text-sm bg-red-700 hover:bg-red-800 text-white font-bold rounded-lg shadow-sm transition-colors" x-text="editMode ? 'Save Changes' : 'Upload Chart'"></button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div x-show="deleteModal" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center bg-black bg-opacity-60 px-4 backdrop-blur-sm transition-opacity">
        <div class="bg-white rounded-2xl p-8 shadow-2xl z-50 w-full max-w-sm transform transition-all relative" @click.away="deleteModal = false">
            ...
            <div class="flex space-x-3 border-t border-gray-100 pt-4">
                ...
                <form :action="'/admin/cid/' + itemId" method="POST" class="flex-1 m-0 p-0 flex">
                    @csrf @method('DELETE')
