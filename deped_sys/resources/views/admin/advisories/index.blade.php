...
<div x-data="{ 
    ...
    openEdit(advisory) {
        ...
        this.formData.removeImage = false;
        this.formData.removePdf = false;
                ...
                <tbody>
                    @forelse($advisories as $advisory)
                        <tr class="hover:bg-gray-50 border-b transition-colors">
                            <td class="p-4 text-sm text-gray-600 font-medium">{{ $advisories->firstItem() + $loop->index }}</td>
                            <td class="p-4 font-semibold text-gray-800">{{ $advisory->title }}</td>
                            ...
                            <td class="p-4 text-sm whitespace-nowrap">
                                @if($advisory->pdf_path)
                                    <a href="{{ asset('storage/' . $advisory->pdf_path) }}" target="_blank" title="{{ basename($advisory->pdf_path) }}" class="text-red-600 font-bold hover:text-red-800 hover:underline flex items-center">
                                        ...
                                        <span class="max-w-[150px] truncate">{{ basename($advisory->pdf_path) }}</span>
                                    </a>
                                @else
                                    <span class="text-gray-400 italic text-xs">N/A</span>
                                @endif
