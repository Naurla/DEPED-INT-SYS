...
<div class="container mx-auto px-4 py-6" x-data="{ 
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="p-4 whitespace-nowrap text-center space-x-3">
                            <button @click="openEdit({{ collect($content)->toJson() }}, '{{ route('admin.curriculum.junior_high.update', $content->id) }}')" class="text-blue-600 hover:text-blue-800 font-bold text-xs uppercase hover:underline">
                                Edit
                            </button>
                            <button @click="openDelete('{{ route('admin.curriculum.junior_high.destroy', $content->id) }}')" class="text-red-600 hover:text-red-800 font-bold text-xs uppercase hover:underline">
                                Delete
                            </button>
                        </td>
                    </tr>
    ...
    <div x-show="uploadModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 px-4 backdrop-blur-sm transition-opacity">
        <div class="bg-white rounded-xl w-full max-w-lg shadow-2xl overflow-hidden" @click.away="uploadModal = false">
            ...
            <form :action="editMode ? editUrl : '{{ route('admin.curriculum.junior_high.store') }}'" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
                @csrf
