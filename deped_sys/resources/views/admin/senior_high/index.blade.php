...
<div class="container mx-auto px-4 py-6" x-data="{ 
    ...
    <div x-show="uploadModal" x-cloak class="fixed inset-0 z-[90] flex items-center justify-center bg-black bg-opacity-60 px-4 backdrop-blur-sm transition-opacity overflow-y-auto">
        <div class="bg-white rounded-xl w-full max-w-5xl shadow-2xl overflow-hidden flex flex-col max-h-[95vh] my-12" @click.away="uploadModal = false">
            
            <!-- Fixed Header -->
            <div class="bg-red-700 px-8 py-5 flex justify-between items-center text-white flex-shrink-0">
                <h3 class="font-bold text-2xl" x-text="editMode ? 'Edit Content' : 'Add New Content'"></h3>
                <button type="button" @click="uploadModal = false" class="hover:text-gray-200 text-4xl font-bold">&times;</button>
            </div>
            
            <form :action="editMode ? editUrl : '{{ route('admin.curriculum.senior_high.store') }}'" method="POST" enctype="multipart/form-data" class="flex flex-col overflow-hidden min-h-0">
                @csrf
                ...
                <div class="p-8 space-y-6 overflow-y-auto custom-scrollbar flex-1">
                    ...
                    <div>
                        ...
                        <textarea name="content" x-model="formData.content" rows="4" placeholder="Briefly describe what this list contains..." 
                                  class="w-full border @error('content') border-red-500 @else border-gray-300 @enderror p-4 text-lg rounded-lg focus:ring-2 focus:ring-red-500 outline-none resize-none transition-shadow"></textarea>
                        @error('content') 
                            <p class="text-red-500 text-base mt-1.5 font-medium">{{ $message }}</p> 
                        @enderror
                    </div>
                    ...
                    <div class="bg-gray-50 p-6 rounded-lg border @error('csv_file') border-red-500 @else border-gray-200 @enderror">
                        ...
                        <input type="file" name="csv_file" accept=".csv,.xlsx,.xls,.doc,.docx,.pdf" 
                        ...
                        <template x-if="editMode && editItem && editItem.csv_path">
                            <div class="mt-3 flex items-center p-3 bg-blue-50 border border-blue-200 rounded-lg">
                                <span class="text-base font-bold text-blue-800 truncate max-w-full" x-text="'Current File: ' + editItem.csv_path.split('/').pop()"></span>
                            </div>
                        </template>
                    </div>
                </div>
                ...
                <div class="bg-gray-50 px-8 py-5 flex flex-row-reverse gap-4 items-center border-t border-gray-200 flex-shrink-0">
                    <button type="submit" class="bg-red-700 hover:bg-red-800 text-white font-bold py-3.5 px-10 rounded-lg shadow-md transition-colors text-lg" x-text="editMode ? 'Save Changes' : 'Create List'"></button>
