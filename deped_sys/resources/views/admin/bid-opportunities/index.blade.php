...
@section('content')
<div class="container mx-auto p-4" x-data="{ addModal: false, editModal: false, deleteModal: false, editBid: null, deleteBid: null }">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-gray-500 text-sm mt-1">
                Provide bid details and attach the necessary files. 
                ...
                <tbody>
                    @forelse($bidOpportunities as $bid)
                        <tr class="hover:bg-gray-50 border-b transition-colors">
                            ...
                            <td class="p-4">
                                @if($bid->pdf_path)
                                    <a href="{{ asset('storage/' . $bid->pdf_path) }}" target="_blank" class="text-red-600 font-bold hover:underline flex items-center text-sm">
                            ...
                            <td class="p-4 text-sm text-gray-500 whitespace-nowrap">{{ $bid->created_at->format('M d, Y') }}</td>
                            
                            <td class="p-4 flex justify-end gap-3 items-center">
                                <button @click="editModal = true; editBid = {{ $bid->toJson() }}" class="text-blue-600 hover:text-blue-800 font-bold text-xs uppercase hover:underline">Edit</button>
                                
                                <button @click="deleteModal = true; deleteBid = {{ $bid->toJson() }}" class="text-red-600 hover:text-red-800 font-bold text-xs uppercase hover:underline">Delete</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="p-6 text-center text-gray-500">No bid opportunities uploaded yet.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div x-show="addModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 px-4 backdrop-blur-sm transition-opacity">
        <div class="bg-white rounded-xl w-full max-w-2xl shadow-2xl overflow-hidden" @click.away="addModal = false">
            <div class="bg-red-700 px-6 py-4 flex justify-between items-center text-white">
                <h3 class="font-bold text-lg">Upload New Bid Opportunity</h3>
                <button type="button" @click="addModal = false" class="hover:text-gray-200 text-2xl font-bold">&times;</button>
            </div>
            
            <form action="{{ route('admin.bid-opportunities.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 p-4 rounded-lg border border-gray-200 mt-2">
                    ...
                    <div>
                        ...
                        <input type="file" name="jpeg_file" accept=".jpeg, .jpg" class="w-full border border-gray-300 p-2 rounded-lg text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-xs file:font-bold file:bg-gray-100 file:text-gray-700 hover:file:bg-gray-200 cursor-pointer">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-400 mt-1">Optional if you provide a JPEG image.</p>
                    </div>
                </div>
    ...
    <div x-show="editModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 px-4 backdrop-blur-sm transition-opacity">
        <div class="bg-white rounded-xl w-full max-w-2xl shadow-2xl overflow-hidden" @click.away="editModal = false">
            <div class="bg-[#a52a2a] px-6 py-4 flex justify-between items-center text-white">
                <h3 class="font-bold text-lg">Edit Bid Opportunity</h3>
                <button type="button" @click="editModal = false" class="hover:text-gray-200 text-2xl font-bold">&times;</button>
            </div>
            
            <form :action="`/admin/procurement/bid-opportunities/${editBid?.id}`" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
                @csrf @method('PUT')
                
                <div>
                    <label class="block text-gray-700 text-sm font-bold mb-1">Title</label>
                    <input type="text" name="title" x-model="editBid.title" class="w-full border border-gray-300 p-2.5 text-sm rounded-lg focus:ring-2 focus:ring-red-500 outline-none" required>
                </div>

                <div>
                    <label class="block text-gray-700 text-sm font-bold mb-1">Description</label>
                    <textarea name="description" x-model="editBid.description" rows="3" 
    ...
    <div x-show="deleteModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 px-4 backdrop-blur-sm transition-opacity">
        <div class="bg-white rounded-xl w-full max-w-md shadow-2xl overflow-hidden" @click.away="deleteModal = false">
            ...
            <div class="p-6">
                <p class="text-gray-700 mb-2">Are you sure you want to delete <span class="font-bold text-gray-900" x-text="deleteBid?.title"></span>?</p>
                ...
                <form :action="`/admin/procurement/bid-opportunities/${deleteBid?.id}`" method="POST" class="flex justify-end gap-3 pt-4 border-t border-gray-100">
                    @csrf @method('DELETE')
