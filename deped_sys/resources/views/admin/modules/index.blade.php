...
<style>
    ...
    /* Subtle scrollbar for the modal content area */
...
<div x-data="{ 
    ...
    editModule: {{ (old('form_type') === 'edit' && $errors->any()) ? Js::from([
        'id' => old('id'),
        'title' => old('title'),
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($modules as $module)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="p-4 text-sm text-gray-600 font-medium text-center align-middle">{{ $modules->firstItem() + $loop->index }}</td>
                            <td class="p-4 font-bold text-gray-900 align-middle">{{ $module->title }}</td>
                            ...
                            <td class="p-4 align-middle">
                                @if($module->file_path)
                                    ...
                                    <a href="{{ asset('storage/' . $module->file_path) }}" target="_blank" class="{{ $docColor }} font-bold hover:underline inline-flex items-center text-xs whitespace-nowrap transition-colors">
                                        ...
                                        {{ Str::limit(basename($module->file_path), 15) }}
                                    </a>
                                @else
                                    <span class="text-gray-400 italic text-[10px]">N/A</span>
                                @endif
                            ...
                            <td class="p-4 align-middle">
                                <div class="flex justify-end gap-3 items-center">
                                    <button @click="openEdit({{ $module->toJson() }})" class="text-blue-600 hover:text-blue-800 font-bold text-xs uppercase hover:underline">Edit</button>
                                    <button @click="confirmDelete({{ $module->id }}, '{{ addslashes($module->title) }}')" class="text-red-600 hover:text-red-800 font-bold text-xs uppercase hover:underline">Delete</button>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="6" class="p-10 text-center text-gray-500 italic">No modules found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div x-show="addModal" x-cloak class="fixed inset-0 z-[90] flex items-center justify-center bg-black bg-opacity-60 px-4 backdrop-blur-sm transition-opacity overflow-y-auto">
        <div class="bg-white rounded-xl w-full max-w-5xl shadow-2xl overflow-hidden flex flex-col max-h-[95vh] my-12" @click.away="if (!isSubmitting) addModal = false">
            ...
            <div class="bg-red-700 px-8 py-5 flex justify-between items-center text-white flex-shrink-0">
                <h3 class="font-bold text-2xl">Upload New Module</h3>
            ...
            <form action="{{ route('admin.modules.store') }}" method="POST" enctype="multipart/form-data" class="flex flex-col overflow-hidden min-h-0" @submit="isSubmitting = true">
                @csrf
                <input type="hidden" name="form_type" value="add">
                
                <div class="p-8 space-y-6 overflow-y-auto custom-scrollbar flex-1">
                    <div>
                        <label class="block text-gray-800 text-lg font-bold mb-2">Module Title <span class="text-red-500">*</span></label>
                        <input type="text" name="title" value="{{ old('form_type') === 'add' ? old('title') : '' }}" required placeholder="Enter module title" class="w-full border @if(old('form_type') === 'add' && $errors->has('title')) border-red-500 @else border-gray-300 @endif p-4 text-lg rounded-lg focus:ring-2 focus:ring-red-500 outline-none transition-shadow" :readonly="isSubmitting">
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="6" required class="w-full border @if(old('form_type') === 'add' && $errors->has('description')) border-red-500 @else border-gray-300 @endif p-4 text-lg rounded-lg focus:ring-2 focus:ring-red-500 outline-none resize-none transition-shadow" :readonly="isSubmitting">{{ old('form_type') === 'add' ? old('description') : '' }}</textarea>
                        @if(old('form_type') === 'add') @error('description') <p class="text-red-500 text-base mt-1.5 font-medium">{{ $message }}</p> @enderror @endif
                    </div>
                    
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50 p-6 rounded-lg border border-gray-200">
                        <div>
                            <label class="block text-gray-800 text-lg font-bold mb-2">Cover Image</label>
                            <input type="file" name="image" accept="image/*" :disabled="isSubmitting" class="w-full border @if(old('form_type') === 'add' && $errors->has('image')) border-red-500 @else border-gray-300 @endif p-3.5 rounded-lg text-lg text-gray-600 file:mr-5 file:py-3 file:px-6 file:rounded-md file:border-0 file:text-base file:font-bold file:bg-red-50 file:text-red-700 hover:file:bg-red-100 cursor-pointer bg-white disabled:opacity-50">
                            @if(old('form_type') === 'add') @error('image') <p class="text-red-500 text-base mt-1.5 font-medium">{{ $message }}</p> @enderror @endif
                        </div>
                        <div>
                            <label class="block text-gray-800 text-lg font-bold mb-2">Document File <span class="text-red-500">*</span></label>
                            <input type="file" name="file" required accept=".pdf,.ppt,.pptx,.doc,.docx,.xls,.xlsx,.csv" :disabled="isSubmitting" class="w-full border @if(old('form_type') === 'add' && $errors->has('file')) border-red-500 @else border-gray-300 @endif p-3.5 rounded-lg text-lg text-gray-600 file:mr-5 file:py-3 file:px-6 file:rounded-md file:border-0 file:text-base file:font-bold file:bg-red-50 file:text-red-700 hover:file:bg-red-100 cursor-pointer bg-white disabled:opacity-50">
                ...
                <div class="bg-gray-50 px-8 py-5 flex flex-row-reverse gap-4 items-center border-t border-gray-200 flex-shrink-0">
                    <button type="submit" :disabled="isSubmitting" :class="{'opacity-75 cursor-wait': isSubmitting, 'hover:bg-red-800': !isSubmitting}" class="bg-red-700 text-white font-bold py-3.5 px-10 rounded-lg shadow-md transition-colors text-lg flex items-center justify-center min-w-[200px]">
                        <span x-show="!isSubmitting">Upload Module</span>
    ...
    <div x-show="editModal" x-cloak class="fixed inset-0 z-[90] flex items-center justify-center bg-black bg-opacity-60 px-4 backdrop-blur-sm transition-opacity overflow-y-auto">
        <div class="bg-white rounded-xl w-full max-w-5xl shadow-2xl overflow-hidden flex flex-col max-h-[95vh] my-12" @click.away="if (!isSubmitting) editModal = false">
            ...
            <div class="bg-red-700 px-8 py-5 flex justify-between items-center text-white flex-shrink-0">
                <h3 class="font-bold text-2xl">Edit Module</h3>
            ...
            <form :action="`/admin/modules/${editModule?.id}`" method="POST" enctype="multipart/form-data" class="flex flex-col overflow-hidden min-h-0" @submit="isSubmitting = true">
                @csrf @method('PUT')
                <input type="hidden" name="form_type" value="edit">
                <input type="hidden" name="id" :value="editModule?.id">
                <input type="hidden" name="existing_image" :value="editModule?.image_path">
                <input type="hidden" name="existing_file" :value="editModule?.file_path">
                ...
                <div class="p-8 space-y-6 overflow-y-auto custom-scrollbar flex-1">
                    <div>
                        <label class="block text-gray-800 text-lg font-bold mb-2">Module Title <span class="text-red-500">*</span></label>
                        <input type="text" name="title" x-model="editModule.title" required class="w-full border @if(old('form_type') === 'edit' && $errors->has('title')) border-red-500 @else border-gray-300 @endif p-4 text-lg rounded-lg focus:ring-2 focus:ring-red-500 outline-none transition-shadow" :readonly="isSubmitting">
                        @if(old('form_type') === 'edit') @error('title') <p class="text-red-500 text-base mt-1.5 font-medium">{{ $message }}</p> @enderror @endif
                    </div>

                    <div>
                        <label class="block text-gray-800 text-lg font-bold mb-2">Description / Details <span class="text-red-500">*</span></label>
                        <textarea name="description" x-model="editModule.description" required rows="6" class="w-full border @if(old('form_type') === 'edit' && $errors->has('description')) border-red-500 @else border-gray-300 @endif p-4 text-lg rounded-lg focus:ring-2 focus:ring-red-500 outline-none resize-none transition-shadow" :readonly="isSubmitting"></textarea>
                        @if(old('form_type') === 'edit') @error('description') <p class="text-red-500 text-base mt-1.5 font-medium">{{ $message }}</p> @enderror @endif
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50 p-6 rounded-lg border border-gray-200">
                        <div>
                            <label class="block text-gray-800 text-lg font-bold mb-2">Replace Cover Image</label>
                            <input type="file" name="image" accept="image/*" :disabled="isSubmitting" class="w-full border @if(old('form_type') === 'edit' && $errors->has('image')) border-red-500 @else border-gray-300 @endif p-3.5 rounded-lg text-lg text-gray-600 file:mr-5 file:py-3 file:px-6 file:rounded-md file:border-0 file:text-base file:font-bold file:bg-red-50 file:text-red-700 hover:file:bg-red-100 cursor-pointer bg-white disabled:opacity-50">
                            @if(old('form_type') === 'edit') @error('image') <p class="text-red-500 text-base mt-1.5 font-medium">{{ $message }}</p> @enderror @endif
                            
                            <template x-if="editModule && editModule.image_path && !removeImage">
                                <div class="mt-3 flex items-center justify-between p-3 bg-blue-50 border border-blue-200 rounded-lg">
                                    <span class="text-base text-blue-800 font-bold truncate max-w-[200px]" x-text="'Current: ' + editModule.image_path.split('/').pop()"></span>
                                    <button type="button" @click="removeImage = true" :disabled="isSubmitting" class="text-red-500 hover:bg-red-100 p-1.5 rounded-lg disabled:opacity-50">
                        ...
                        <div>
                            <label class="block text-gray-800 text-lg font-bold mb-2">Replace Document File</label>
                            <input type="file" name="file" accept=".pdf,.ppt,.pptx,.doc,.docx,.xls,.xlsx,.csv" :disabled="isSubmitting" class="w-full border @if(old('form_type') === 'edit' && $errors->has('file')) border-red-500 @else border-gray-300 @endif p-3.5 rounded-lg text-lg text-gray-600 file:mr-5 file:py-3 file:px-6 file:rounded-md file:border-0 file:text-base file:font-bold file:bg-red-50 file:text-red-700 hover:file:bg-red-100 cursor-pointer bg-white disabled:opacity-50">
                            @if(old('form_type') === 'edit') @error('file') <p class="text-red-500 text-base mt-1.5 font-medium">{{ $message }}</p> @enderror @endif
                            
                            <template x-if="editModule && editModule.file_path && !removeFile">
                                <div class="mt-3 flex items-center justify-between p-3 bg-red-50 border border-red-200 rounded-lg">
                                    <span class="text-base text-red-800 font-bold truncate max-w-[200px]" x-text="'Current: ' + editModule.file_path.split('/').pop()"></span>
                                    <button type="button" @click="removeFile = true" :disabled="isSubmitting" class="text-red-500 hover:bg-red-100 p-1.5 rounded-lg disabled:opacity-50">
                                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                    </button>
                                </div>
                            </template>
    ...
    <div x-show="deleteModal" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center bg-black bg-opacity-60 px-4 backdrop-blur-sm transition-opacity">
        <div class="bg-white rounded-3xl shadow-2xl z-50 w-full max-w-md transform transition-all relative overflow-hidden p-8" @click.away="if (!isSubmitting) deleteModal = false">
            ...
            <div class="text-center">
                <h3 class="text-2xl font-bold text-gray-900 mb-2">Delete Module?</h3>
            ...
            <div class="flex gap-3">
                ...
                <form :action="`/admin/modules/${deleteId}`" method="POST" class="flex-1 m-0 p-0 flex" @submit="isSubmitting = true">
                    @csrf 
                    @method('DELETE')
