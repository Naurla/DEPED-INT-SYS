        ...
        <div class="bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] border border-gray-100 overflow-hidden" 
            ...
            <div class="pt-10 pb-6 px-8 text-center border-b border-gray-50 bg-gray-50/30">
                ...
                <h2 class="text-2xl font-black text-gray-900 tracking-tight uppercase" 
                    x-text="view === 'login' ? 'Admin Portal' : (view === 'forgot' ? 'Reset Password' : 'New Password')">
                </h2>
            ...
            <div class="p-8 md:p-10">
                ...
                {{-- LOGIN FORM --}}
                <form action="{{ route('admin.login') }}" method="POST" x-show="view === 'login'" x-transition.opacity.duration.300ms>
                    @csrf
                    <div class="mb-5">
                        <label class="block text-gray-700 text-xs font-bold uppercase tracking-wider mb-2">Email Address</label>
                        <input type="email" name="email" value="{{ old('email') }}" required class="w-full bg-gray-50 border border-gray-300 px-4 py-3 rounded-lg focus:ring-2 focus:ring-[#a52a2a] focus:bg-white outline-none transition-colors text-sm">
                    </div>
                    <div class="mb-2">
                    ...
                    <div class="flex justify-end mb-8 mt-3">
                        <button type="button" @click="view = 'forgot'" class="text-sm font-semibold text-blue-600 hover:text-blue-800 transition-colors focus:outline-none">Forgot Password?</button>
                    </div>
                    
                    <button type="submit" class="w-full bg-[#a52a2a] text-white font-bold py-3.5 rounded-lg hover:bg-red-800 transition-all shadow-md hover:shadow-lg hover:-translate-y-0.5 uppercase tracking-widest text-sm">
                        Sign In
                    </button>
                </form>

                {{-- FORGOT PASSWORD FORM --}}
                <form action="/admin/password/email" method="POST" x-show="view === 'forgot'" x-cloak x-transition.opacity.duration.300ms>
                    @csrf
                    <p class="text-sm text-gray-600 mb-6 leading-relaxed">Enter your registered email address and we will send you a code to reset your password securely.</p>
                    ...
                    <button type="submit" class="w-full bg-[#a52a2a] text-white font-bold py-3.5 rounded-lg hover:bg-red-800 transition-all shadow-md hover:shadow-lg hover:-translate-y-0.5 uppercase tracking-widest text-sm mb-6">
                        Send Reset Code
                    </button>
                    
                    <div class="text-center flex flex-col gap-3 border-t border-gray-100 pt-6">
                        <button type="button" @click="view = 'reset'" class="text-sm font-semibold text-blue-600 hover:text-blue-800 transition-colors focus:outline-none">I already have a code</button>
                        <button type="button" @click="view = 'login'" class="text-sm font-semibold text-gray-500 hover:text-gray-900 transition-colors focus:outline-none">Back to Login</button>
                    </div>
                </form>

                {{-- RESET PASSWORD FORM --}}
                <form action="/admin/password/reset" method="POST" x-show="view === 'reset'" x-cloak x-transition.opacity.duration.300ms>
                    @csrf
                    <div class="mb-5">
                        <label class="block text-gray-700 text-xs font-bold uppercase tracking-wider mb-2">Email Address</label>
                        <input type="email" name="email" value="{{ old('email') }}" required class="w-full bg-gray-50 border border-gray-300 px-4 py-3 rounded-lg focus:ring-2 focus:ring-[#a52a2a] focus:bg-white outline-none transition-colors text-sm">
                    ...
                    <div class="mb-5">
                        ...
                        <input type="text" name="code" value="{{ old('code') }}" required class="w-full bg-gray-50 border border-gray-300 px-4 py-3 rounded-lg focus:ring-2 focus:ring-[#a52a2a] focus:bg-white outline-none transition-colors text-sm tracking-widest" placeholder="• • • • • •">
                    </div>
                    <div class="mb-5">
                    ...
                    <div class="mb-8">
                        ...
                        <input type="password" name="password_confirmation" required class="w-full bg-gray-50 border border-gray-300 px-4 py-3 rounded-lg focus:ring-2 focus:ring-[#a52a2a] focus:bg-white outline-none transition-colors text-sm">
                    </div>
                    
                    <button type="submit" class="w-full bg-blue-600 text-white font-bold py-3.5 rounded-lg hover:bg-blue-700 transition-all shadow-md hover:shadow-lg hover:-translate-y-0.5 uppercase tracking-widest text-sm mb-6">
                        Update Password
                    </button>
                    
                    <div class="text-center border-t border-gray-100 pt-6">
                        <button type="button" @click="view = 'forgot'" class="text-sm font-semibold text-gray-500 hover:text-gray-900 transition-colors focus:outline-none">Back to email verification</button>
                    </div>
                </form>
