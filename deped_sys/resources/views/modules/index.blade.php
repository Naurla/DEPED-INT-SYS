...
<div class="bg-gray-100 border-b border-gray-200 w-full overflow-hidden">
    <div class="container mx-auto px-4 md:px-20 max-w-10xl py-3 text-xs sm:text-sm text-gray-600 overflow-x-auto whitespace-nowrap hide-scroll">
        ...
        <span>K to 12</span>
        <span class="mx-2">></span>
        <span class="text-gray-900 font-bold">ALS Modules</span>
    </div>
</div>
        ...
        <div class="space-y-16">
            @forelse($items as $item)
                <div class="group transition-all duration-300">
                    ...
                    <a href="{{ route('k12.als.modules.show', $item->id) }}" class="block">
                        <h2 class="text-xl md:text-[1.35rem] font-extrabold text-gray-900 leading-snug uppercase group-hover:text-[#003366] transition-colors mb-4">
                            {{ $item->title }} __ {{ Str::limit(strip_tags($item->description), 100) }}
                        </h2>
                    </a>
                    
                    <p class="text-gray-600 text-[15px] font-medium leading-relaxed mb-6 pr-10">
                        {{ $item->title }} - {{ Str::limit(strip_tags($item->description), 200) }}
                    </p>
                    
                    <div class="flex items-center gap-6">
                        <a href="{{ route('k12.als.modules.show', $item->id) }}" class="inline-block border border-gray-300 text-gray-600 px-6 py-2 text-xs font-bold uppercase tracking-widest hover:bg-gray-50 hover:text-gray-900 transition-colors rounded-sm shadow-sm">
                            Read More
                        </a>
