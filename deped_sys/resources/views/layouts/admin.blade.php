...
<body class="bg-gray-100 flex h-screen overflow-hidden" x-data="{ sidebarOpen: true }">
    ...
    <aside class="bg-[#a52a2a] text-white transition-all duration-300 flex flex-col shadow-xl z-20 h-screen sticky top-0 shrink-0" 
        ...
        <nav class="flex-grow p-4 space-y-2 text-sm overflow-y-auto mt-2 custom-scrollbar">
            ...
            <div x-data="{ dropdownOpen: {{ request()->routeIs('admin.curriculum.*') ? 'true' : 'false' }} }" class="relative mt-2">
                <button @click="dropdownOpen = !dropdownOpen" 
                    class="w-full flex items-center justify-between px-4 py-3 rounded-lg transition-colors {{ request()->routeIs('admin.curriculum.*') ? 'bg-red-800 font-bold shadow-inner border border-red-700/50' : 'hover:bg-red-700' }}">
                ...
                <div x-show="dropdownOpen && sidebarOpen" x-collapse x-cloak class="pl-11 pr-4 py-3 mt-1 space-y-3 bg-red-900/30 rounded-lg shadow-inner">
                    ...
                    <div x-data="{ subOpen: {{ request()->routeIs('admin.curriculum.index') ? 'true' : 'false' }} }" class="space-y-1">
    ...
    <div x-show="subOpen" x-collapse class="pl-3 space-y-2 border-l border-red-700 mt-2">
        ...
        <a href="{{ route('admin.faq.index') }}" class="block text-xs transition-all {{ request()->routeIs('admin.faq.*') ? 'text-white font-bold' : 'text-gray-300 hover:text-white' }}">FAQ</a>
    </div>
</div>
                    </div>

                    <a href="#" class="block py-1 text-sm text-gray-200 hover:text-white hover:font-bold transition-all">Learning Materials</a>
